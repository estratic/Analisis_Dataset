...
# Ejercicio 1
## Descripcion del dataset ¿Por que es importante y que pregunta/problema pretende responder?
...
##2.2 ¿Los datos contienen ceros o elementos vacios? ¿Y valores extremos? ¿Cómo gestionarás cada uno de estos casos?
...
```{r}
surveyMentalHealth_clean_v1<-subset(surveyMentalHealth, surveyMentalHealth$Age>16 & surveyMentalHealth$Age<75)
nrow(surveyMentalHealth_clean_v1)
```

Una vez eliminados los datos atípicos de la variable edad y teniendo en cuenta que esta es cuantitativa procedemos a obtener los siguientes datos:

```{r}
summary(surveyMentalHealth_clean_v1$Age)
```

```{r}
boxplot(surveyMentalHealth_clean_v1$Age)
```

```{r}
#Media Aritmetica 
Media_Age<-mean(surveyMentalHealth_clean_v1$Age)
#Mediana
Mediana_Age<-median(surveyMentalHealth_clean_v1$Age)
#Media Recortada
Media_Recortada_Age<-mean(surveyMentalHealth_clean_v1$Age, trim=0.05)
#Desviación estándar
Desviacion_estandar_Age<-sd(surveyMentalHealth_clean_v1$Age)
#Rango Intercuartilico (RIC)
RIC_Age<-IQR(surveyMentalHealth_clean_v1$Age)
#Desviación Absoluta Respecto de la Mediana
Desviacion_Absoluta_Mediana_Age<-mad(surveyMentalHealth_clean_v1$Age)
#Tabla 
...
Esta es una variable cualitativa. Asi que por definición de la variable deberí ahber 2 niveles (Male y Female).
```{r}
levels(surveyMentalHealth_clean_v1$Gender)
```
...
```{r}
surveyMentalHealth_clean_v1$Gender<-as.character(surveyMentalHealth_clean_v1$Gender)
surveyMentalHealth_clean_v1$Gender<-replace(surveyMentalHealth_clean_v1$Gender,surveyMentalHealth_clean_v1$Gender=="Cis Female"|surveyMentalHealth_clean_v1$Gender=="cis-female/femme"|surveyMentalHealth_clean_v1$Gender=="f"|surveyMentalHealth_clean_v1$Gender=="femail"|surveyMentalHealth_clean_v1$Gender=="Femake"|surveyMentalHealth_clean_v1$Gender=="female"|surveyMentalHealth_clean_v1$Gender=="Female"|surveyMentalHealth_clean_v1$Gender=="Female "|surveyMentalHealth_clean_v1$Gender=="Female (cis)"|surveyMentalHealth_clean_v1$Gender=="Female (trans)"|surveyMentalHealth_clean_v1$Gender=="Woman"|surveyMentalHealth_clean_v1$Gender=="woman","F")

surveyMentalHealth_clean_v1$Gender<-replace(surveyMentalHealth_clean_v1$Gender,surveyMentalHealth_clean_v1$Gender=="Male"|surveyMentalHealth_clean_v1$Gender=="male"|surveyMentalHealth_clean_v1$Gender=="Cis Male"|surveyMentalHealth_clean_v1$Gender=="m"|surveyMentalHealth_clean_v1$Gender=="Mail"|surveyMentalHealth_clean_v1$Gender=="Make"|surveyMentalHealth_clean_v1$Gender=="male leaning androgynous"|surveyMentalHealth_clean_v1$Gender=="Malr"|surveyMentalHealth_clean_v1$Gender=="msle"|surveyMentalHealth_clean_v1$Gender=="ostensibly male, unsure what that really means"|surveyMentalHealth_clean_v1$Gender=="something kinda male?"|surveyMentalHealth_clean_v1$Gender=="Androgyne"|surveyMentalHealth_clean_v1$Gender=="cis male"|surveyMentalHealth_clean_v1$Gender=="Cis Man"|surveyMentalHealth_clean_v1$Gender=="Guy (-ish) ^_^"|surveyMentalHealth_clean_v1$Gender=="maile"|surveyMentalHealth_clean_v1$Gender=="Mal"|surveyMentalHealth_clean_v1$Gender=="Male (CIS)"|surveyMentalHealth_clean_v1$Gender=="Male-ish"|surveyMentalHealth_clean_v1$Gender=="Man"|surveyMentalHealth_clean_v1$Gender=="Male ","M")

surveyMentalHealth_clean_v1$Gender<-replace(surveyMentalHealth_clean_v1$Gender,surveyMentalHealth_clean_v1$Gender=="A little about you"|surveyMentalHealth_clean_v1$Gender=="Agender"|surveyMentalHealth_clean_v1$Gender=="All"|surveyMentalHealth_clean_v1$Gender=="Enby"|surveyMentalHealth_clean_v1$Gender=="fluid"|surveyMentalHealth_clean_v1$Gender=="Genderqueer"|surveyMentalHealth_clean_v1$Gender=="Nah"|surveyMentalHealth_clean_v1$Gender=="Neuter"|surveyMentalHealth_clean_v1$Gender=="non-binary"|surveyMentalHealth_clean_v1$Gender=="p"|surveyMentalHealth_clean_v1$Gender=="queer"|surveyMentalHealth_clean_v1$Gender=="queer/she/they"|surveyMentalHealth_clean_v1$Gender=="Trans woman"|surveyMentalHealth_clean_v1$Gender=="Trans-female",NA)

surveyMentalHealth_clean_v1$Gender<-as.factor(surveyMentalHealth_clean_v1$Gender)
```
...
```{r}
surveyMentalHealth_clean<-subset(surveyMentalHealth_clean_v1, surveyMentalHealth_clean_v1$Gender!="NA")
nrow(surveyMentalHealth_clean)
```

***HAY QUE DISCUTIR SI ES LO MAS CORRECTO***
...
```
 Consideramos *Maybe* como una unica categoria agrupada a la nueva categoria *Si* 

```{r}
surveyMentalHealth_clean$mental_health_consequence<- combineLevels(surveyMentalHealth_clean$mental_health_consequence,levs = c('Maybe'), newLabel = c("Yes"))
...
```{r}
surveyMentalHealth_clean$phys_health_consequence<- combineLevels(surveyMentalHealth_clean$phys_health_consequence,levs = c("Maybe"), newLabel = c("Yes"))
...
```{r}
surveyMentalHealth_clean$coworkers<- combineLevels(surveyMentalHealth_clean$coworkers,levs = c("Some of them"), newLabel = c("Yes"))
...
```{r}
surveyMentalHealth_clean$supervisor<- combineLevels(surveyMentalHealth_clean$supervisor,levs = c("Some of them"), newLabel = c("Yes"))
...
```{r}
surveyMentalHealth_clean$mental_health_interview<- combineLevels(surveyMentalHealth_clean$mental_health_interview,levs = c("Maybe"), newLabel = c("Yes"))
...
```{r}
surveyMentalHealth_clean$phys_health_interview<- combineLevels(surveyMentalHealth_clean$phys_health_interview,levs = c("Maybe"), newLabel = c("Yes"))
...
```{r}
surveyMentalHealth_clean$mental_vs_physical<- combineLevels(surveyMentalHealth_clean$mental_vs_physical,levs = c("Don't know"), newLabel = c("Yes"))
...
Comencemos con el analisis de la normalidad para el conjunto de valores de la variable Edad que consideran que la organización da mayor importancia a la salud mental vs salud fisica.
...
```{r}
#Diagrama de caja (Boxplot)
boxplot(Age_Mental, main="Box Plot Age Salud Fisica")
```
En un gráfico de Boxplot podemos estudiar la simetría, detectar outlier e incluso contrastar algunas hipótesis de la distribución. El gráfico fracciona los datos en 4 partes de igual frecuencia, es decir, cada grupo contiene mas o menos el mismo número de observaciones. Pero la ocupación de estos es diferente.El primer grupo(desde el valor mas pequeño hasta Q1) los valores de la variable Age va desde 18 hasta 27. El último grupo va (desde Q3 hasta el máximo valor) desde 37 hasta 61. Podemos observar que la longitud desde el mínimo hasta Q1 es diferente a la de Q3 al máximo, por lo que podemos decir que no existe simetria con respecto a la mediana, por tanto podemos hablar de asimetria. El 50% de los individuos observados tienen Age entre Q1 y Q3.
...
```{r chunck31}
hist(Age_Mental ,main="Edad individuos con Organizacion mas valor a la Salud Mental",
...
plot(den_Age_Mental ,main="Densidad de la edad de los individuos observados")
...
plot(den_Age_Fisica ,main="Densidad de la edad de los individuos observados")
...
hist(Age_Mental ,main="Edad individuos Organizacion Salud Mental",
     col="gold",freq=FALSE)
...
```{r}
#Superposición de las gráficas
hist(Age_Fisica ,main="Edad Individuos Organizacion Salud Fisica",
     col="gold",freq=FALSE)
lines(den_Age_Mental ,col="blue",lwd=4)
```
...
```{r}
ks.test(x=Age_Mental,"pnorm", mean(Age_Mental), sd(Age_Fisica))
```
...
Procedemos a rechazar la hipotesis nula de normalidad ya que en todos los test obtenemos un $p_{value}$<0.05
...
Por lo tanto, tenemos un conjunto de la variable edad donde se ha recibido tratamiento asociado a la salud mental y otro conjunto de la variable edad donde no se ha recibido tratamiento asociado a la salud mental.
Comenzamos con el análisis de la normalidad para el conjunto de valores de la variable Age que han recibido tratamiento relacionado con la salud mental.

```{r}
#Conjunto de valores de la variable Edad que consideran que la organización da mayor importancia a la salud mental vs salud fisica.
Age_Tratamiento<-subset(surveyMentalHealth_clean$Age, surveyMentalHealth_clean$treatment=="Yes")
```
Y el conjunto de valores de la variable Age que no han recibido tratamiento relacionado con la salud física.
```{r}
...
```
En un gráfico de Boxplot podemos estudiar la simetría, detectar outlier e incluso contrastar algunas hipótesis de la distribución. El gráfico fracciona los datos en 4 partes de igual frecuencia, es decir, cada grupo contiene mas o menos el mismo número de observaciones. Pero la ocupación de estos es diferente.El primer grupo(desde el valor mas pequeño hasta Q1) los valores de la variable Age va desde 18 hasta 27. El último grupo va (desde Q3 hasta el máximo valor) desde 37 hasta 72. Podemos observar que la longitud desde el mínimo hasta Q1 es diferente a la de Q3 al máximo, por lo que podemos decir que no existe simetria con respecto a la mediana, por tanto podemos hablar de asimetria. El 50% de los individuos observados tienen Age entre Q1 y Q3.

```{r}
#Diagrama de caja (Boxplot)
boxplot(Age_NTratamiento, main="Box Plot Age Sin Tratamiento")
```
En un gráfico de Boxplot podemos estudiar la simetría, detectar outlier e incluso contrastar algunas hipótesis de la distribución. El gráfico fracciona los datos en 4 partes de igual frecuencia, es decir, cada grupo contiene mas o menos el mismo número de observaciones. Pero la ocupación de estos es diferente.El primer grupo(desde el valor mas pequeño hasta Q1) los valores de la variable Age va desde 18 hasta 27. El último grupo va (desde Q3 hasta el máximo valor) desde 35 hasta 75. Podemos observar que la longitud desde el mínimo hasta Q1 es diferente a la de Q3 al máximo, por lo que podemos decir que no existe simetria con respecto a la mediana, por tanto podemos hablar de asimetria. El 50% de los individuos observados tienen Age entre Q1 y Q3.
...
```
Vamos a utilizar tambien el gráfico de los cuantiles teóricos (Graficos Q-Q). 
```{r}
...
```
Podemos tener en cuenta tambien el test de normalidad de Jarque-Bera
...
```
Podemos concluir que el test si haya diferencias significativas entre las varianzas de los dos grupos.
...
Vamos a realizar el estudio ANOVA de un factor (one-way ANOVA o independent samples ANOVA) para investigar si existen diferencias en la edad entre los individuos que han tenido tramiento o no de salud mental.
...
En este caso hemos encontrado cambio significativa de la variable treatment (si han recibido o no tratamiento relacionado con la salud mental) ya que el $p_{value}$ ha sido menor que 0.05

Continuamos con el estudio ANOVA de un factor (one-way ANOVA o independent samples ANOVA) para investigar si existen diferencias en la edad entre los individuos que consideran que las empresas le dan mas importancia a la salud mental vs salud fisica.
